<ul class="flex-row hidden-xs hidden-sm">
    <li class="flex-col">
        <a href="<?=SITEURL?>/contact-us/">
            <div class="circle">
                <?php echo get_template_part('template-parts/icons/icon','1') ?>
            </div>
        </a>
        <span class="fade-down">$0 <span>(Out Of Pocket)</span>
            <div class="br-md">
                hail repair in
                Odessa & Midland</div>
        </span>
    </li>
    <li class="flex-col">
        <a href="<?=SITEURL?>/contact-us/">
            <div class="circle">
                <?php echo get_template_part('template-parts/icons/icon','2') ?>
            </div>
        </a>
        <span class="fade-down">$200 <div class="br-md">Gift Card</div></span>
    </li>
    <li class="flex-col">
        <a href="<?=SITEURL?>/contact-us/">
            <div class="circle">
                <?php echo get_template_part('template-parts/icons/icon','3') ?>
            </div>
        </a>
        <span class="fade-down">We Pay Your<br />
            Deductible<br />
            $0 <span>(Out Of Pocket)</span></span>
    </li>

    <li class="flex-col">
        <a href="<?=SITEURL?>/contact-us/">
            <div class="circle">
                <div>
                    <?php echo get_template_part('template-parts/icons/icon','4') ?>
                </div>
            </div>
        </a>
        <span class="fade-down">Free Pick Up<br />
            & Delivery</span>
    </li>

    <li class="flex-col">
        <a href="<?=SITEURL?>/contact-us/">
            <div class="circle">
                <div>
                    <?php echo get_template_part('template-parts/icons/icon','5') ?>
                </div>
            </div>
        </a>
        <span class="fade-down">Free<br />
            Rental Car</span>
    </li>

    <li class="flex-col">
        <a href="<?=SITEURL?>/contact-us/">
            <div class="circle">
                <div>
                    <?php echo get_template_part('template-parts/icons/icon','6') ?>
                </div>
            </div>
        </a>
        <span class="fade-down">We Work With<br />
            All Insurances</span>
    </li>

    <li class="flex-col">
        <a href="<?=SITEURL?>/contact-us/">
            <div class="circle">
                <div>
                    <?php echo get_template_part('template-parts/icons/icon','7') ?>
                </div>
            </div>
        </a>
        <span class="fade-down">$100 For<br />
            Referrals</span>
    </li>
</ul>